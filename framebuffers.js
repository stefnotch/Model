...
/*global gl vaoExt lightRot outlines*/
...
//Sets ShaderProg to 1, not exactly intended.
/*function require(string) {
  return 1;
}
var ShaderProg = require("./script.js");
*/

function pixelPicker(renderFunction) {
  this.pixel = new Uint8Array(4); //4->RGBA
  ...
  var vShader = `
    ...
    void main(void){
      ...
      v_boneID = a_boneWeight > 0.5 ? a_bone.x : a_bone.y;
    }`;
...
function pickPixel(objectsToDraw, viewMatrix, boneMat) {
  ...
  //Read the pixel
  gl.readPixels(0, 0, 1, 1, gl.RGBA, gl.UNSIGNED_BYTE, this.pixel); //gl.RGBA or gl.RGB
